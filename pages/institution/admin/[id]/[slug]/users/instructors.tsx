...
const InviteInstructor = () => {
  ...
  return (
    <Stack>
      <Title order={2}>Invite Instructor</Title>
      <form onSubmit={form.onSubmit((values) => handleSendInvite())}>
        <Stack className='h-100' justify='center'>
          <TextInput label={'Email'} radius={'md'} placeholder='Enter email of the instructor' {...form.getInputProps('email')} />
    ...
    <div>
      <Stack>
        <Box>
          ...
          <Text size="sm">Those who accepted the instructor invite.</Text>
        </Box>
        <CustomDataTable
          url={API_ENDPOINTS.INSTITUTION_INSTRUCTORS}
          method={'GET'}
          defaultFilters={{
            page: 1,
            limit: '10',
            ...
            fields: "instructor, user,full_name, email, username, profile, avatar, phone_number, date_joined, last_login, is_active"
          }}
          useNext={false}
            ...
            },
            {
              ...
              render: (item: any) => (
                <Group p={0} style={{
                  ...
                  <Avatar radius="md" src={item?.instructor?.user?.profile?.avatar} alt={item?.user?.full_name} />
                  <Stack gap={2}>
                    <Text size="sm">{item?.instructor?.user?.full_name}</Text>
                    <Text size="xs" c={'dimmed'}>{item?.instructor?.user?.email}</Text>
                    <Text size="sm">{item?.instructor?.user?.username}</Text>
                  </Stack>
                </Group>
              )
            },
            {
              accessor: 'instructor.user.profile.phone_number',
              title: 'Phone Number',
              width: '180px',
            },
            {
              ...
              width: '180px',
              render: (item: any) => (
                <Button size="xs" w={'80px'} variant="light" radius={'xl'} color={item?.instructor?.user?.is_active ? 'green' : 'yellow'}>{item?.instructor?.user?.is_active ? 'Yes' : 'No'}</Button>
              )
            },
            {
              ...
              width: '250px',
              render: (item: any) => (
                <Text size='sm'>{toDate(item?.instructor?.user?.date_joined, true)}</Text>
              )
            },
            {
              accessor: 'last_login',
              ...
              width: '250px',
              render: (item: any) => (
                <Text size='sm'>{toDate(item?.instructor?.user?.last_login, true)}</Text>
              )
            }
          ]}
          filterFields={[
...
Instructors.PageLayout = InstitutionWrapper
